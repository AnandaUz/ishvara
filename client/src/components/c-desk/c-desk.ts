import "./c-desk.scss";
import html from "./c-desk.html?raw";
...
export class Desk {
  ...
  init(weightLogs: IWeightLog[] = []) {
    ...
    const totalDayCount = Math.ceil(
      (toDay.getTime() - minT.getTime()) / (1000 * 60 * 60 * 24),
    );
          ...
          if (diffDays > 1) {
            ...
            for (let i = 1; i < diffDays; i++) {
              const newDate = new Date(
                prevDate.getTime() + i * (1000 * 60 * 60 * 24),
              );
              mas.push({
                date: newDate,
                weight: prevWeight + step * i,
                isDashed: true,
              });
            }
          }
  ...
  render() {
    ...
    const W = (this.weightData.totalDayCount + 30 * 12) * this.scale.forDay;
    const H =
      (this.weightData.max - this.weightData.min + 10) * this.scale.forWeight;
    ...
    this.dYY =
      (Options.view_endWeight - this.weightData.max - 1) * this.scale.forWeight;
    ...
    bgs[1] = {
      ...
      size: {
        w: r2 * 2,
        h:
          (Options.view_endWeight - Options.view_startWeight) *
          this.scale.forWeight,
      },
      repeat: "no-repeat",
    };
    ...
    for (let ii = 0; ii < this.graphs.length; ii++) {
      ...
      if (gr) {
        for (let i = 0; i < gr.length - 1; i++) {
          ...
          if (curr && next) {
            const y1 =
              (this.weightData.max - curr?.weight) * this.scale.forWeight;
            const y2 =
              (this.weightData.max - next?.weight) * this.scale.forWeight;
            ...
            line.setAttribute(
              "stroke",
              Options.paths_styles[ii]?.color || "#b42020ff",
            );
            line.setAttribute(
              "stroke-width",
              Options.paths_styles[ii]?.strokeWidth + "" || "1",
            );
  ...
  drawTangent(graph: TGraphPoint[], pointIndex: number) {
    ...
    const wPrev = loessGraph[iClamped - 1]?.weight ?? 0;
    const wNext = loessGraph[iClamped + 1]?.weight ?? 0;

    // скорость в кг/день (отрицательная = похудение)
    const dw_per_day = (wNext - wPrev) / 2;

    // 3. Координаты точки касания в SVG
    const wCurr = loessGraph[iClamped]?.weight ?? 0;
    const px = (iClamped - 1) * this.scale.forDay;
    const py = (this.weightData.max - wCurr) * this.scale.forWeight;

    // 4. Наклон в SVG-пикселях
    // dy/dx в пикселях: dy = -dw * scaleWeight (минус т.к. Y перевёрнут), dx = scaleDay
    const slope_svg = (-dw_per_day * this.scale.forWeight) / this.scale.forDay;
  ...
  setStartPosition() {
    ...
    this.destPosition.y =
      (-(this.weightData.max - middleW) + weightInDest / 2) *
      this.scale.forWeight;
    this.setDestPosition();
  }
  private initDrag() {
